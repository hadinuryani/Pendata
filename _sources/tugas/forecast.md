...
2. **Bentuk Data Training**
   ...
   Setelah tahap agregasi harian (`resample('D')`) dan pembuatan matriks latih menggunakan `create_train_X_y()`, data training dibentuk menjadi matriks fitur prediktor ($X$) dan variabel target ($y$). Berikut adalah contoh representasi data training per barisnya:
   ...
   - **Input (Fitur X)**:
     - `lag_1` sampai `lag_7`: Nilai historis total permintaan listrik harian dari 1 hingga 7 hari sebelumnya.
     - `Temperature`: Rata-rata suhu harian pada hari target yang akan diprediksi (variabel eksogen).
   - **Output (Fitur y)**:
     - `Demand`: Nilai aktual total permintaan listrik pada hari target yang ingin diprediksi.

3. **Apa itu lag?**

   **Lag** (keterlambatan/selang waktu) dalam data deret waktu (*time series*) adalah nilai historis (masa lalu) dari variabel target itu sendiri yang dijadikan sebagai fitur input (prediktor). 
   
   Sebagai contoh, jika target yang ingin kita prediksi adalah nilai permintaan pada waktu $t$ ($y_t$), maka:
   - `lag_1` adalah nilai target pada waktu $t-1$ (kemarin).
   - `lag_2` adalah nilai target pada waktu $t-2$ (dua hari yang lalu).
   - ...
   - `lag_7` adalah nilai target pada waktu $t-7$ (seminggu yang lalu).
   
   Pemanfaatan lag membantu model mendeteksi korelasi serial (*autocorrelation*), yaitu kecenderungan nilai saat ini dipengaruhi oleh nilainya di masa lalu.

4. **Jelaskan proses analysis yang dilakukan dari kasus diatas**

   Proses analisis yang dilakukan untuk memprediksi permintaan energi dan memahami perilaku model terdiri dari beberapa langkah berurutan:

   - **Langkah 1: Mengimpor Pustaka (Library)**  
     Mengimpor pustaka utama untuk manipulasi data (`pandas`), visualisasi (`matplotlib.pyplot`), interpretasi model (`shap`, `sklearn.inspection`), model regresi (`LGBMRegressor`), dan pembuatan model peramalan runtun waktu (`skforecast`).

   - **Langkah 2: Menyiapkan dan Mengagregasikan Data**  
     - Mengunduh data permintaan listrik Victoria (`vic_electricity`) yang beresolusi setengah-jam.
     - Mengubah data setengah-jam menjadi frekuensi harian (`resample('D')`) dengan menjumlahkan total `Demand` dan merata-ratakan `Temperature` per hari.
     - Membagi data menjadi **Data Training** (data historis sebelum 22 Desember 2014) untuk melatih model, dan **Data Testing** (mulai 22 Desember 2014) untuk evaluasi prediksi.

   - **Langkah 3: Membangun dan Melatih Forecaster**  
     - Membuat model peramal rekursif (`ForecasterRecursive`) menggunakan algoritma `LGBMRegressor` (LightGBM) dengan parameter `lags=7`.
     - Melatih (*fit*) model tersebut dengan data training (`y = Demand` dan `exog = Temperature`).

   - **Langkah 4: Model-Specific Feature Importance**  
     - Mengekstrak tingkat pentingnya fitur bawaan model (*feature importance*) berbasis pohon keputusan (*mean decrease impurity*) menggunakan metode `get_feature_importances()`.
     - Menampilkan kontribusi tiap fitur prediktor (lags dan suhu) dalam diagram batang horizontal, menunjukkan bahwa variabel suhu dan nilai lag hari sebelumnya merupakan prediktor yang sangat penting.

   - **Langkah 5: Permutation Feature Importance**  
     - Membuat matriks input latih menggunakan `create_train_X_y()`.
     - Mengukur kepentingan fitur secara acak dengan teknik *permutation importance*. Teknik ini bekerja dengan cara mengacak isi dari suatu kolom fitur lalu mengukur seberapa besar penurunan performa model (skor $R^2$). Fitur yang ketika diacak menyebabkan penurunan performa paling besar dinilai sebagai fitur yang paling penting.

   - **Langkah 6: Partial Dependence Plot (PDP)**  
     - Membuat plot ketergantungan parsial (PDP) untuk menganalisis hubungan fungsional antara nilai fitur individual (seperti `Temperature` atau `lag_1`) dengan respons prediksi rata-rata model.
     - Membantu memahami tren (apakah pengaruhnya naik, turun, atau non-linear). Misalnya, kita dapat melihat pola pengaruh suhu terhadap peningkatan permintaan listrik.

   - **Langkah 7: Menjelaskan Model dengan SHAP Values**  
     - Menggunakan `shap.TreeExplainer` untuk menghitung nilai kontribusi marginal Shapley untuk setiap observasi.
     - **Global Interpretability**: Membuat *summary plot* berbentuk batang untuk mengetahui kepentingan fitur secara global, serta berbentuk titik-titik (*dot*) untuk menganalisis arah pengaruh (misalnya, nilai suhu tinggi/merah berkorelasi dengan SHAP values positif/meningkatkan prediksi permintaan).
     - **Interaksi Fitur**: Menganalisis ketergantungan dan interaksi antar-fitur melalui *dependence plot*.
     - **Local Interpretability**: Membuat *force plot* untuk menjelaskan satu prediksi individual tertentu guna menunjukkan kontribusi detail fitur pendorong (panah merah) dan penarik (panah biru) terhadap hasil prediksi dibanding nilai baseline model (*expected value*).
     - **Forecast Explainability**: Menggunakan `create_predict_X()` untuk mendapatkan matriks fitur data masa depan, lalu menghitung SHAP values pada data masa depan tersebut untuk menerangkan mengapa model memproyeksikan tren forecast tertentu langkah demi langkah.